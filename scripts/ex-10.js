...
function test(dictionary) {
  ...
  for (let key in dictionary) {
    ...
    let en = prompt(
      uz + ' ("stop loop" bilan sinovni to\'xtatishingiz mumkin.)',
    );
    if (en == "stop loop" || en == null) {
      console.log(
        `Jami ${s + k} ta savoldan ${s} ( ${(s / (s + k)) * 100} % ) tasiga to'ri javob berdingiz.`,
      );
      console.log(
        `Jami ${s + k} ta savoldan ${k} tasiga noto'ri javob berdingiz.`,
      );
      break;
    }
