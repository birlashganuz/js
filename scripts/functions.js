...
while (isNaN(n) || n < 4 || n > 50) {
  n = +prompt(
    "Yulduzchalar sonini natural sonda kiriting (kiritilgan son 3 dan katta 51 dan kichkina bo'lishi shart):",
  );
}
if (n % 2 == 1) {
  n += 1;
...
function stars(n = 3) {
  console.log(
    `Siz kiritgan ma\'lumot asosida ${n + 1}x${n + 1} kvadrat shakllantirildi.`,
  );
  let space = "( )";
  star = "(" + star + ")";
  for (let i = 0; i <= n; i++) {
